...
class TCSRepositoryUnited(val requestMode:Boolean, val appWidgetManager: AppWidgetManager?, val appWidgetID: Int?): KoinComponent {
            ...
            override fun onResponse(call: Call<TCSResponse>, response: Response<TCSResponse>) {
                ...
                if (!requestMode) {
                    ...
                    if (buyUSD == 0.0 || buyEUR == 0.0 || buyGBP == 0.0) {
                        Log.wtf("WidgetUpd", "Failed")
                        getCurrentTCS()
                    } else {
                        Log.wtf("WidgetUpd", "Success")
